...
void main() {
    ...
    0.08 * sin(5.0 * theta - 0.08 * u_time) +
    0.03 * sin(7.0 * theta + 0.04 * u_time) +
    0.02 * sin(8.0 * theta - 0.06 * u_time) +
    0.04 * sin(11.0 * theta + 0.1 * u_time));
    float l = length(pos);
    ...
    pos.x += f * sin(pos.y / 20.0 + u_time * 3.141592653 / 30.0) * 10.0;
    pos.y += f * sin(pos.x / 20.0 + u_time * 2.718281828 / 120.0) * 5.0;
    gl_FragColor = texture2D(u_texture, (pos + u_bossPosition) / u_screenSize);
    gl_FragColor.rgb += vec3(0.0, 0.0, 0.3) * f;
}